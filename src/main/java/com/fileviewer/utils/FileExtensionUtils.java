...
/**
 * Mapa de colores por extensión de archivo para la visualización del treemap.
 *
 * <p>
 * Asocia cada extensión conocida a un color RGB base. Al solicitar el color con
 * {@link #getColor(String)}, se aplica una leve variación aleatoria para
 * diferenciar visualmente nodos del mismo tipo.</p>
 */
public class FileExtensionUtils {
    ...
    static {
        ...
        for (String ext : videoExts) {
            addColor(ext, 255, 100, 255);
        }

        // Audio
        String[] audioExts = {".m4a", ".aiff", ".au", ".mid", ".midi", ".mp3", ".wav", ".wma"};
        for (String ext : audioExts) {
            addColor(ext, 100, 255, 200);
        }

        // Comprimidos
        String[] zipExts = {".zip", ".rar"};
        for (String ext : zipExts) {
            addColor(ext, 215, 1, 130);
        }

        // Lenguajes
        String[] javaExts = {".java", ".jar"};
        for (String ext : javaExts) {
            addColor(ext, 255, 180, 120);
        }
    ...
    /**
     * Registra un color base para una extensión.
     */
    ...
    /**
     * Obtiene un color RGBA (con leve variación aleatoria) para la extensión
     * indicada. Si la extensión no está registrada, retorna un gris por
     * defecto.
     *
     * @param extension extensión del archivo (por ejemplo {@code ".java"})
     * @return cadena CSS {@code rgba(r, g, b, a)}
     */
    ...
    /**
     * Genera una cadena RGBA con variación aleatoria sutil.
     */
